...
// -------------------------------------------------------------------------------
  ...
  if (ec == 0) {
    ...
    for(i=0; i<(64*256*(num_blocks+1) + 64); i++) {
      dif = read(RBF,&dat,4);
      if (!(dif)) {
        dat = 0xFFFFFFFF;
      }
  ...
  if (ec == 0) {
    ...
    while (cp == 1) {
      ...
      if ((ct - st) > 5) {
	      printf ("\nFAILURE --> Programming did not complete after 5 sec\n");
	      cp = 0;
	      ec = 4;
      }
    }
  ...
  if (ec == 0) {
    ...
    for(i=0; i<(64*256*(num_blocks+1)); i++) {
      ...
      if (!(dif)) {
        ...
        //# -------------------------------------------------------------------------------
        //# Wait for Flash Data to be ready
        //# -------------------------------------------------------------------------------
        st = lt = time(NULL);
        cp = 1;
        ...
        while (cp == 1) {
          ...
          if ((ct - st) > 30) {
            printf ("\nFAILURE --> Read Data not ready after 1/2 min\n");
            cp = 0;
            ec = 16;
          }
        }
